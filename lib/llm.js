...
if (q.includes("billing")) {
  ...
  // Try to extract billing ID (number)
  const match = q.match(/\d+/);

  return {
    intent: {
      type: "billingDetails",
      billingId: match ? match[0] : null
    },
    explanation: "Fetching billing details"
  };
}
...
export async function generateAnswer(userQuery, queryResult) {
  ...
  switch (queryResult.type) {
    ...
    case "summaryStats":
      ...
      case "billingDetails":
  return queryResult.billing
    ? `Billing ${queryResult.billing.id} found.`
    : "Billing not found.";
