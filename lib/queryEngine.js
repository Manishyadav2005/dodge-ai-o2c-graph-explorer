...
export function executeQuery(intent) {
  ...
  switch (intent.type) {
    ...
    case 'topProductsByBilling': {
     ...
     return {
  type: 'result',
  title: 'Top Products by Billing',

  data: sorted.map(p => ({
    id: p.material,
    label: p.material,
    value: p.billingCount
  })),

  highlightIds: sorted.map(p => p.material),

summary: `Top ${sorted.length} products with highest billing activity. Highest product has ${sorted[0]?.billingCount} billing documents.`
}
    }
...
return {
  type: 'result',
  title: `Billing Flow: ${docId}`,

  data: [
    { label: 'Customer', value: customer?.customerId, name: customer?.name },
    { label: 'Sales Order', value: salesOrder?.salesOrder },
    { label: 'Delivery', value: delivery?.deliveryDocument },
    { label: 'Billing', value: billing.billingDocument },
    ...(paymentRecs.length
      ? paymentRecs.map(p => ({
          label: 'Payment',
          value: p.accountingDocument
        }))
      : [])
  ...
  highlightIds: [
    ...
    ...paymentRecs.map(p => p.accountingDocument)
  ].filter(Boolean),

  summary: `Complete flow traced for billing document ${docId}.`
};
      }
      ...
      if (so) {
        ...
        return {
  type: 'result',
  title: `Sales Order Flow: ${docId}`,

  data: [
    { label: 'Customer', value: customer?.customerId, name: customer?.name },
    { label: 'Sales Order', value: so.salesOrder },
    ...relDeliveries.map(d => ({
      label: 'Delivery',
      value: d.deliveryDocument
    })),
    ...relBillings.map(b => ({
      label: 'Billing',
      value: b.billingDocument
    }))
  ].filter(item => item.value),
  ...
  highlightIds: [
    customer?.customerId,
    so.salesOrder,
    ...relDeliveries.map(d => d.deliveryDocument),
    ...relBillings.map(b => b.billingDocument)
  ].filter(Boolean),

  summary: `Complete flow traced for sales order ${docId}.`
};
      }
  ...
  return {
    ...
    // 👇 UI ke liye clean data
    data: customerList,

    // 👇 Graph highlight ke liye
    highlightIds: customerList.map(c => c.id),

    summary: `Found ${customerList.length} customers in the dataset.`,
  };
}
